...
#define KCol 7
#define KBtnW (SCREEN_WIDTH * 0.8)/7
#define KBtnH (SCREEN_WIDTH * 0.8)/7
#define KMaxCount 37
#define KBtnTag 100
#define KTipsW (SCREEN_WIDTH * 0.2)
...
- (void)creatControl
{
    ...
    weekLabel.font = [UIFont systemFontOfSize:14 * autoSizeScaleX];
    ...
    yearLabel.font = [UIFont systemFontOfSize:19.0f * autoSizeScaleX];
    ...
    monthLabel.font = [UIFont systemFontOfSize:22.0f * autoSizeScaleX];
    [tipsView addSubview:monthLabel];
    self.monthLabel = monthLabel;
    
    //日期标签
    UILabel *dayLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(monthLabel.frame) + 5 * autoSizeScaleY, KTipsW, 90 * autoSizeScaleY)];
    dayLabel.textColor = [UIColor whiteColor];
    dayLabel.backgroundColor=[UIColor clearColor];
    dayLabel.textAlignment = NSTextAlignmentCenter;
    dayLabel.font = [UIFont systemFontOfSize:45.0f * autoSizeScaleX];
    [tipsView addSubview:dayLabel];
    self.dayLabel = dayLabel;
    
    CGFloat yearBtnW = (SCREEN_WIDTH * 0.8) * 0.3;
    CGFloat monthbtnW = (SCREEN_WIDTH * 0.8) * 0.2;
    CGFloat todayBtnW = (SCREEN_WIDTH * 0.8) * 0.3;
    CGFloat padding = (SCREEN_WIDTH - KTipsW - yearBtnW - monthbtnW - todayBtnW - KBtnW * 2) * 0.25;
    
    //年份按钮
    HWOptionButton *yearBtn = [[HWOptionButton alloc] initWithFrame:CGRectMake((SCREEN_WIDTH * 0.2), 0, yearBtnW, KBtnH)];
    ...
    //上一月
    UIButton *preBtn = [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetMaxX(yearBtn.frame) + 5, (KBtnH - ((SCREEN_WIDTH * 0.8) * 0.1))/2, (SCREEN_WIDTH * 0.8) * 0.1, (SCREEN_WIDTH * 0.8) * 0.1)];
    ...
    //下一月
    UIButton *nextBtn = [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetMaxX(monthBtn.frame), (KBtnH - ((SCREEN_WIDTH * 0.8) * 0.1))/2, (SCREEN_WIDTH * 0.8) * 0.1, (SCREEN_WIDTH * 0.8) * 0.1)];
    ...
    backTodayBtn.titleLabel.font = [UIFont systemFontOfSize:13.0f];
    [backTodayBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [backTodayBtn setTitle:@"今天" forState:UIControlStateNormal];
...
- (void)setShowTimePicker:(BOOL)showTimePicker
{
    ...
    if (showTimePicker) {
        ...
        _dayLabel.frame = CGRectMake(0, CGRectGetMaxY(_monthLabel.frame) + 5 * autoSizeScaleY, KTipsW, 90 * autoSizeScaleY);
        _timePicker.frame = CGRectMake(0,KBtnH * 8 - 88, KTipsW , 88);
    }else {
        _timePicker.hidden = YES;
        _dayLabel.frame = CGRectMake(0, CGRectGetMaxY(_monthLabel.frame) + 20 * autoSizeScaleY, KTipsW, 90 * autoSizeScaleY);
    }
}
...
- (void)reloadData
{
    ...
    _yearLabel.text = [NSString stringWithFormat:@"%ld", (long)_year];
    _monthLabel.text = [NSString stringWithFormat:@"%ld月", (long)_month];
    _yearBtn.title = [NSString stringWithFormat:@"%ld年", (long)_year];
    _monthBtn.title = [NSString stringWithFormat:@"%ld月", (long)_month];
        ...
        if (i < firstDay - 1 || i > totalDays + firstDay - 2) {
            ...
            if (_year == _currentYear && _month == _currentMonth) {
                if (btn.tag - KBtnTag - (firstDay - 2) == _currentDay) {
                    ...
                    _dayLabel.text = [NSString stringWithFormat:@"%ld", (long)_day];
                }
            }else {
                if (i == firstDay - 1) {
                    btn.selected = YES;
            ...
            btn.enabled = YES;
            [btn setTitle:[NSString stringWithFormat:@"%d", i - (firstDay - 1) + 1] forState:UIControlStateNormal];
        }
    }
}
...
{
    NSString *time1 = _timeArray[component][row];
    if (component == 0) {
        _hour = [time1 integerValue];
    } else if (component == 1) {
        _minute = [time1 integerValue];
    }
}
...
- (void)sureBtnOnClick
{
    ...
    NSString *date;
    if (_showTimePicker) {//%02ld:%02ld 时分 (long)_hour, (long)_minute
        date = [NSString stringWithFormat:@"%ld.%02ld.%02ld", (long)_year, (long)_month, (long)_day];
    }else {
        date = [NSString stringWithFormat:@"%ld.%02ld.%02ld", (long)_year, (long)_month, (long)_day];
    }
...
{
    [UIView animateWithDuration:0.3f animations:^{
        self.frame = CGRectMake(0, [UIScreen mainScreen].bounds.size.height, SCREEN_WIDTH , self.bounds.size.height);
    }];
}
