...
//    当系统消息出现UIKeyboardWillShowNotification和UIKeyboardWillHideNotification消息就会调用我们的keyboardWillShow和keyboardWillHide方法。
    [[self.passwordF rac_textSignal] subscribeNext:^(id x) {
        ...
        if (self.passwordF.text.length > 6) {
            ...
            self.passwordF.text = [NSString stringWithFormat:@"%@",x];
        }
        ...
        if (self.passwordF.text.length == 0) {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:@"input_PasswordNotification" object:nil userInfo:@{@"password":x}];
